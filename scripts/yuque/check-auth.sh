...
DEFAULT_YUQUE_DIR="/Users/mac/Desktop/yuque"

if [[ -n "${YUQUE_DATA_DIR:-}" ]]; then
  YUQUE_DIR="$YUQUE_DATA_DIR"
elif [[ -f "$DEFAULT_YUQUE_DIR/.env.local" ]]; then
  YUQUE_DIR="$DEFAULT_YUQUE_DIR"
else
  YUQUE_DIR="$DEFAULT_YUQUE_DIR"
fi

ENV_FILE="$YUQUE_DIR/.env.local"
if [[ ! -f "$ENV_FILE" && -f "$PROJECT_ROOT/.env.local" ]]; then
  ENV_FILE="$PROJECT_ROOT/.env.local"
fi

if [[ ! -f "$ENV_FILE" ]]; then
  echo "❌ 未找到 $YUQUE_DIR/.env.local，请复制 .env.local.example 并填入 Cookie"
  exit 1
fi

# 用 Node 解析 .env.local，避免 Cookie 中的 = 号破坏 shell 解析
eval "$(
  node -e "
    const fs = require('fs');
    const env = {};
