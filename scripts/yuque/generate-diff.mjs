...
function walkMarkdownRelPaths(yuqueRoot) {
  const repos = listRepoMirrorDirs();
  const results = [];
...
function main() {
  ...
  if (!backupRoot || !fs.existsSync(backupRoot)) {
    console.error('❌ 未找到备份目录。请先运行: npm run yuque:backup');
    process.exit(1);
  }

  const currentFiles = walkMarkdownRelPaths(yuqueRoot);
  const diffs = [];
