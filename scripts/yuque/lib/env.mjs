...
export const DEFAULT_YUQUE_DATA_DIR = '/Users/mac/Desktop/yuque';

const RESERVED_DIRS = new Set(['_meta', '_backup', '_reports']);
...
export function resolveEnvFilePath() {
  const candidates = [
    ...
    DEFAULT_YUQUE_DATA_DIR && path.join(DEFAULT_YUQUE_DATA_DIR, '.env.local'),
  ...
  return candidates[0] || path.join(DEFAULT_YUQUE_DATA_DIR, '.env.local');
}
...
export function getCookieHeader(env = loadEnvLocal()) {
  ...
  if (env.YUQUE_COOKIE) {
    return `_yuque_session=${env.YUQUE_COOKIE}`;
  }
  return null;
}
...
export function validateYuqueAuth(env = loadEnvLocal()) {
  const cookie = getCookieHeader(env);
  if (!cookie && !env.YUQUE_COOKIE) {
    return '未配置 YUQUE_COOKIE 或 YUQUE_COOKIE_FULL';
  }
  if (!getSessionValue(env)) {
    return 'Cookie 缺少 _yuque_session。请在浏览器 F12 → Application → Cookies → yuque.com 复制完整 Cookie（须包含 _yuque_session=... 和 yuque_ctoken=...）';
  }
  return null;
}
