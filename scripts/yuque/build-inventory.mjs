...
function scanRepoMirror({ book, mirrorDir, defaultGroup }) {
  ...
  for (const file of files) {
    ...
    const { slug, group, book: urlBook } = extractDocMeta(content, book, defaultGroup);
    const resolvedBook = urlBook || book;
    ...
    const relPath = `${resolvedBook}/${innerRel}`;
...
function main() {
  ...
  if (repos.length === 0) {
    console.error(
      `❌ 未找到知识库镜像。请先运行:\n   npm run yuque:sync-all\n   或 npm run yuque:sync`,
    );
    process.exit(1);
  }
