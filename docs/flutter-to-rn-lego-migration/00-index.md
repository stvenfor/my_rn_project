...
3. 阅读 **`09-agent-programming-playbook.md`**，按 Slice Brief → 审计 → 实现 → 测试闭环 → 交互验证组织每轮 Agent 工作。
...
7. 每阶段按 `04-cursor-execution-spec.md` 产出 Delivery Report + UX 证据。
8. 提交审查前按 **`08-acceptance-checklist.md`** 自检（权威）；`05-acceptance-and-qa-plan.md` 作补充说明。
